...
Likert scale variables (and hence data) are widely utilised in research, they are useful for getting participants to rate things or to provide an average quantity as a response in situations where asking for the exact quantity may be problematic. How can asking for an exact quantity be problematic? Well, take the example below that we expand upon that relates to how much water one drinks per day. Very few people drink the exact same amount of water each day, so asking participants "How many (250ml) glasses of water do you drink per day" and getting a response of "3" is typically pointless, there is likely substantial measurement error here and if they drank 3 glasses yesterday (anomaly or otherwise) but 4 glasses the day before, is the response of 3 not just outright incorrect?

The below is nothing ground breaking. We went in search of a concise, succinct, and accurate way to display (specifically) pre-post Likert data, and this is where we are currently at.
...
We'll return to birectional Likert data in a future post, for now we will look at ordinal Likert data.
...
We have some `pre` and `post` response data, a `group` identifier (intervention or control), and then some labelled response columns.
...
The above isn't perfect, one could argue that there is no need to duplicate the figure headings and that the legend could be handled better. Is a figure ever perfect?
...
As mentioned, we'll return to bidirection Likert data in a future post.
