...
When we have pre-post data (focusing on continuous, approximately normally distributed data here), there three basic approaches analysts may reach for:

-   **Change score analysis**
    -   the outcome data for each participant is reduced to one data point (a change score: post value - pre value)
    -   can just be analysed with a t.test or as the outcome variable of a linear regression model with the treatment variable as the exposure of interest (on the right hand side (RHS) of the model)
-   **Linear regression (ANCOVA framework)**
    -   the data is structured as one row per participant, with two variables representation the outcome (one for pre, one for post)
...
-   **Linear mixed effects models**
    ...
    -   the data is structured as two rows per participant, with one row for the pre data and one row for the post data (there will be a column here that represents time, with values `pre` and `post`)
        -   both the pre and post measurements are on the left hand side (LHS) of the model, with an interaction between the treatment variable (as the exposure of interest) and time on the RHS of the model

There are a number of nuanced variations on these methods to be aware of. This article is not stricticly focused on randomised controlled trials, for which a lot of [methodological literature](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-021-01323-9) [addressing this](https://psycnet.apa.org/doiLanding?doi=10.1037%2Fh0025105) [decision exists](https://onlinelibrary.wiley.com/doi/10.1002/sim.2682) - it is intended to be a light, applied look at model implementation.
...
-   The post timepoint values are (on average) \~20 units higher than at the pre timepoint values.
-   **\[Perhaps of most interest\]** The post values for treatment group are (on average) \~30 units higher than the control group.
