...
Potentially, it is easier to think about when we should not consider parallel computing. For parallel computing to be workable, your overall job needs to be something that can be broken down into [independent]{.underline} tasks, that is, into tasks that can each be completed with [no dependence on the input or output from another task]{.underline}. So, any job where the tasks must completed in sequential order can not be executed (or sped up) with parallel computing - this might be sequential (dependent) steps of data processing (need to standardise the variables BEFORE we identify (and tag) the extreme values) or sequential steps of computation (matrix algebra within a regression model).
...
As it relates to data analysis, parallel computing requires that we can divvy up tasks in a standalone way, and (generally) assemble the output from those tasks in a sensible way. An examples
