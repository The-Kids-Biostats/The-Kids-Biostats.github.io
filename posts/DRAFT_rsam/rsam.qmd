...
`rsam` is hosted only on Github and can be installed easily using the [`remotes`](https://cran.r-project.org/web/packages/remotes/index.html) package. Noting the package code hasn't changed (or had updates) for 8 years (at the time or writing.

::: {.column-margin title="Note"}
The package code hasn't actually changed (e.g., been updated) for 8 years - at the time of writing.
:::
