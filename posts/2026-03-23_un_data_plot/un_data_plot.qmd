...
headers = c("Authorization" = "Bearer <XXXXXXXXX>") # add your token here
source("un_data_source_helper.R")

# Source indicator codes
indicators <- callAPI("/indicators/") # 86 indicators
...
1.  pulling a table of the indicators into a data frame
    -   these are the variable names/IDs and we need to know the IDs for those particular variables/data we want
2.  pulling a table of the locations into a data frame
    -   we want to request the data for a limited number of locations (countries) to reduce the size of the data request, so we also need to know the IDs for those countries
3.  pulling the current population size for all countries
    -   a quick way to find the IDs for the larger countries, as I knew we wanted to explore some of the larger countries data
4.  viewing (filtering, arranging etc) the population size data and creating a vector of the IDs (self explantory)
...
-   `startAge=24&endAge=2569` these relate to the numerator and denominator boundaries we are interested in, there are different ratios available for different numerator and denominator boundaries and combinations `&`
...
-   `geom_label_repel()` is from the `ggrepel` package
:::
...
![The first plot](images/fig2_gtp_1.png){width="100%" align="center"}

You see it too?

![The first plot - zoom 1](images/fig3_gtp_1.png){align="center"}

Oh dear!

![The first plot - zoom 2](images/fig4_gtp_1.png){align="center"}
...
There is a wealth of really interesting data available at the [United Nations Population Division](https://www.un.org/development/desa/pd/) [data portal api](https://population.un.org/dataportalapi/index.html). It is fantastic that they make it all so readily available.
