...
All of which was correct. It was a good post, and there was a lot of repetition in the code. Which got me thinking about the age old battle in programming...

# The case for code

There are (always) a lot of people out there learning to use R, and a lot of them love experimenting with R's great visualisation tools. And a lot of those people are still learning to be code savvy. I know this as I cross paths with this type of user multiple times a week, copy and pasting examples from blogs (like this) and using trial and error to learn. It is this large cohort of users that one can appeal to by sharing verbose code.
...
-   It is crystal clear what and where the code can (successfully) be customised, for example labels, as opposed to wondering if your customisations (passed into a function) will be honoured by the function or if your edits within the function will break it, and
...
Also, in no time at all, one can copy that code into an LLM, and ask it to provide helper text to explain what is happening at each stage. 

That is, of course, assuming the author didn't take the time to do that themselves ...
...
-   It can remove a barrier to entry, for those who don't want to clutter the `.qmd` files or risk any copying errors, they can load the function in and get on with their day,
...
When deciding between using basic code or wrapping that code it into functions, it's important to balance simplicity and efficiency - and think of your audience. For R-based visualisations and reproducible research workflows used within in Quarto documents, showing basic code does offer transparency and fosters learning, making it ideal for teaching or exploratory analyses. On the other hand, functions enhance reusability and scalability, streamlining workflows for complex or repeated tasks.
