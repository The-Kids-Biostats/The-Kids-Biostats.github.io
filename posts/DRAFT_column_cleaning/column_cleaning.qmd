...
This post will demonstrate how two new functions within [`thekidsbiostats`](https://github.com/The-Kids-Biostats/thekidsbiostats) package can be used to streamline and expedite the process of standardising and labelling columns. Specifically, the following functions are introduced:
...
2. simultaneously retain human-readable labels for presentation/reporting.
...
We can either directly add to, or modify, the `dict` object (see above) or copy-paste the output into our code and then add/edit/update column names and labels as needed (see below):
...
As the number of columns in your dataset increases, editing/maintaining this within your code may be unwieldy. The option is also available to manage this via an external file (e.g., csv, rds, xlsx), by specifying `file`:
...
And then we would read the csv back in after having (externally to R) modified the column names and labels as necessary:
...
And after these steps, we're don't have this...
