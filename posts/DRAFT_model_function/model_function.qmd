...
Below, we present the current way we present output from a linear regression within our `.html` reports.
...
To demonstrate, we will use a slightly modified dataset containing Birth Weight data (read more with `?MASS::birthwt`) from the `MASS` package (`MASS::birthwt`).
...
In this, we have passed (piped) the data into the first argument, specified the outcome variable in the next argument, specified the exposure of interest in the next argument, and finally specified the remainder of the models formula (that is, any covariates we are looking to have in our model).
