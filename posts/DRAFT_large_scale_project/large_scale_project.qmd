...
For those unfamiliar with Quarto, note that documents can be written and rendered from either a **visual** view or a **source** view:
...
-   Use of a document template for report writing that includes a run date in the header and session info at the end (See `thekidsbiostats::create_template()`)
