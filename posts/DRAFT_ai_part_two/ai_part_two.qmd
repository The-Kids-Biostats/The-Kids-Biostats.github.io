...
In summary, 10 different biostatistics-related problems were posed to ChatGPT 3.5 and subsequently ChatGPT 4.0. It was given three attempts to correctly answer each question. The results are below:
...
We attempted to reproduce the results of Question 2, which was:
...
Graphically, we can highlight the are of the distribution we are interested in calculating - this will be helpful later to compare with ChatGPT's responses. 
...
Below is our prompt history with ChatGPT 4.0 as we attempted to reproduce the results from the paper - namely, that it took two or three attempts to get the correct answer.
...
Interestingly, the *binom* package is not used!
...
We see that the AI has doubled down and suggesting we have misunderstood the question. It describes the problem back to us perfectly, with one exception:

"The focus should be on the cumulative probability of having *up to* 19 survivors."

This is the first time anything it has said has matched the calculation it did. 

## Third Attempt

The final attempt is up for debate on where it is correct or not:
...
A similar paper to Example 2, testing responses to a set of 70 medical questions:
https://onlinelibrary.wiley.com/doi/full/10.1111/iej.13985

Despite the humourous title, a fascinating paper on where Artificial Intelligence is, well, intelligent:
