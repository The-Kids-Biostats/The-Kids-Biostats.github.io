...
The purpose of the package can be summed up as to provide:
...
data.frame(
  ...
  Description = c(
    ...
    "Creates a folder structure for a new project, as well as an R Project file and basic R scripts",
    "Creates a Quarto document from one of several template (extention) choices",
    "Like dplyr::case_when but the output variable is of class factor, ordered based on the order entered into the case_when statement",
    "Rounds all numeric columns in a data frame or tibble, keeping trailing zeroes",
    "Rounds all values in a vector, keeping trailing zeroes",
    ...
    "Generates a collection of informative model output in a structured list when provided data and model specification",
    "Generates a collection of informative model output in a structured list when provided with an existing model object",
    "Creates a formatted flextable with \"The Kids\" themed colours and fonts",
    "Applies \"The Kids\" theme to a ggplot object, including colours, fonts and changes to other elements for a cleaner look",
    "Like thekids_theme, but for historical projects done under the Telethon Kids Institute style guide"
  ) 
...
In this instance, we create a project called "package_demo". Both the parent directory and the Rproj file take this name. We choose the "targets" extension, which means our directory starts with a "\_targets.R" file and a "scripts" folder (Had we chosen the "basic" extension, neither of these would be included.)
...
The purpose of `create_template()` is to generate ready-to-populate Quarto templates with The Kids styling already applied. Two templates are possible using the `ext_name` parameter: html and word, corresponding to the format that will be rendered by the Quarto document.
...
Briefly, this function provides a standardised list of outputs for commonly used models in R. This includes the model itself, diagnostics, marginal means, and clean tables of typical model output (coefficients, confidence intervals, p-values etc). To see how to display these nicely in a Quarto html document, see the blog post above. 

## round_vec and round_df

These two functions are examples where the sole purpose is keep code concise and to the point. Often when we present figures whether it is in plots, tables or in written form, we wish to keep trailing zeroes when rounding numbers to a x decimal places.
...
In a similar vein to the above function, fct_case_when is essentially case_when from the `dplyr` package but with one handy addition: the resulting vector is a factor where the levels are ordered in the same order they are defined within the case_when statement. 
...
Using functions that automatically apply a set of formatting options to plots and tables saves time, allowing us to focus on the analysis and interpretation. Code also looks a lot cleaner when those 10 lines of ggplot per plot are automated away. Importantly, these functions also ensure a polished and consistent appearance across our team, so that output looks the same no matter who generated it.
...
`thekids_table()` produces tables styled with The Kids branding and is powered by the `flextable` package. This includes by default applying the Barlow font, compact formatting, and zebra-striping for readability, though these can all be altered/disabled via parameters.
...
The thekidsbiostats package significantly simplifies repetitive tasks, standardizes formatting, and enhances workflows. While tailored for The Kids Research Institute, many of its features can be readily adapted to other contexts, providing a robust framework for data analysis and reporting. 
...
We hope you'll give the package a try and find some or all of the functions useful in your own workflow. Please feel free to contact us at biostatistics@thekids.org.au if you encounter any bugs or wish to suggest additional features for any of our functions. 
