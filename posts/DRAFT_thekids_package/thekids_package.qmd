...
The package (and all of its code!) are available here: [https://github.com/The-Kids-Biostats/thekidsbiostats](https://github.com/The-Kids-Biostats/thekidsbiostats).
...
In this instance, we create a project called "package_demo". Both the parent directory and the R project (.Rproj) file take this name. We choose the "targets" extension, which means our directory starts with a "\_targets.R" file and a "scripts" folder (had we chosen the "basic" extension, neither of these objects would be created).
...
The `thekidsbiostats` package significantly simplifies repetitive tasks, standardizes formatting, and enhances workflows. While tailored for use at The Kids Research Institute Australia, many of its features can be readily adapted to other contexts, providing a robust framework for data analysis and reporting.
