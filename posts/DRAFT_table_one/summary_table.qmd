...
+ **Cons**
  ...
  + Specifying any variable levels or summary variables must be done outside of any tabling pipelines.
  + All formatting must be done in a custom rendering function.
    + Specifying this rendering function is not necessarily straightforward, and relies still on specific functions of the `table1` package.
